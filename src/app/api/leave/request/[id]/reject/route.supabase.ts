...
async function rejectLeaveRequestHandler(
  ...
  try {
    ...
    console.log('DEBUG: validatedData =', validatedData);

    if (!validatedData) {
      // Try parsing body manually as fallback
      try {
        const body = await req.json();
        console.log('DEBUG: manually parsed body =', body);
        ...
        console.log('DEBUG: using manually parsed reason:', reason.substring(0, 50));
      } catch (parseError) {
        console.error('DEBUG: Failed to parse body manually:', parseError);
        return apiError('Request validation failed - could not parse request body', 400);
      }
    } else {
      reason = validatedData.reason;
      console.log('DEBUG: using validated reason from middleware:', reason.substring(0, 50));
    }
